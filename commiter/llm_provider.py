    ...
    def _make_request(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            if "response" in response:
                ...
                console.print(
                    f"[red]Ollama API error: {response.get('error', 'Unknown error')}[/red]"
                )
                return None
            ...
            console.print(
                "[red]curl not found. Please install curl to use Ollama provider.[/red]"
            )
            return None

    def generate_commit_messages(
        self, diff_content: str, max_choices: int = 3
    ) -> List[str]:
    ...
    def _parse_commit_messages(self, response: str, max_choices: int) -> List[str]:
        ...
        for line in lines:
            ...
            if not line or line.startswith(
                ("Here are", "Based on", "Options:", "1.", "2.", "3.", "4.", "5.")
            ):
                continue
            ...
            # Additional sanitization: remove inline markdown anywhere in the line
            # - strip list numbering/bullets at start
    ...
    def __init__(self, provider: str = "ollama", model: str = "llama3.2", **kwargs):
        ...
        if provider == "ollama":
            self.client = OllamaProvider(
                model=model, base_url=kwargs.get("base_url", "http://localhost:11434")
            )
        else:
